...
module Db {
	...
	export class ObjC<T> {
		...
		dbInit(url :string, db :Db) {
			...
			for (var i = 0; i < evts.length; i++) {
				var ev = <Event<any>>this.events[evts[i]];
				ev.dbInit(url,db);
			}
		}
		...
		static readRef<X>(data:any, db:Db) :ObjC<X> {
			...
			if (typeof(data) === 'object') {
				...
				for (var k in data) {
					if (k == '_ref') continue;
					var proj = data[k];
					var event = <AccessEvent>objc.events[k];
	...
	export class ValueEvent<T> extends Event<T> {
	...
	export class ListEvent<T> {
		...
		constructor(private name :string) {
			this.add = new AddedListEvent<T>(name);
			this.remove = new Event<T>(name);
			this.modify = new Event<T>(name);
			this.all = new AddedListEvent<T>(name);
		...
		subQuery() :ListEvent<T> {
			var ret = new ListEvent<T>(this.name);
			if (this._ctorD) {
	...
	export function TNone<T>(h :EventHandler<T>) :EventHandler<T> {
		return h;
	}
